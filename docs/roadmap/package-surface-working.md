...
| `vendors/walmart` Orders parsing | Source repair, 2026-09-09 | Normalize omitted/null collections only when metadata explicitly reports zero matches and no continuation. Preserve explicit terminal arrays and reject malformed or contradictory responses. Normalize digit-string page-size echoes; omitted/null/zero echoes use the request or cursor page size without rewriting the cursor. Response errors retain bounded Zod causes plus content-free stage issue codes. Tracked in `ai-tools-wok`; package release and host adoption remain separate. |
| `vendors/wayfair` | Source repair, 2026-09-11 | 40 tools unchanged. Credentials now project to JSON Schema without overrides; document storage moves from `auth.artifacts` to client options or `ToolContext.extras.artifacts`, retaining host and object providers. See [binding migration](../vendors/wayfair.md#document-storage-binding), tracked in `ai-tools-p6r.5`. Production defaults and sandbox support remain; public product-addition/group-media conflicts stay excluded. Package release and host adoption remain separate; no live certification claimed. |
| `vendors/sps-commerce` | Source repair, 2026-09-11 | 25 tools and 34 client methods unchanged. Both bearer-token and client-credentials auth alternatives now serialize without overrides; artifacts and document-origin permissions move to client options or `ToolContext.extras`. See [binding migration](../vendors/sps-commerce.md), tracked in `ai-tools-p6r.6`. Transaction v5, public label/slip APIs and Community Submission remain covered; private batch APIs and unspecified EDI order/invoice schemas remain excluded. Package release and host adoption are separate. |
